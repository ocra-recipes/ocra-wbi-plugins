...
// namespace sequence{
    void PoseTest::doInit(ocra::Controller& ctrl, ocra::Model& model)
    {
        ...
        taskManagers["tmLeftHandPose"]      = new ocra::SegPoseTaskManager(ctrl, model, "leftHandPoseTask", "l_hand", ocra::XYZ, Kp, Kd, wLeftHandTask, endingDispd);
    }

    void PoseTest::doUpdate(double time, ocra::Model& state, void** args)
    {
        ocra::SegPoseTaskManager*   tmp_tmLeftHandPose = dynamic_cast<ocra::SegPoseTaskManager*>(taskManagers["tmLeftHandPose"]);
