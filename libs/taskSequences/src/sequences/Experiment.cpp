...
#include <taskSequences/sequences/Experiment.h>
...
Experiment::Experiment()
{
    ...
    rootLogFilePathPrefix += "Experiment";
...
void Experiment::doInit(wocra::wOcraController& ctrl, wocra::wOcraModel& model)
{
    ...
    */
    ...
    std::cout << "Begin moving to grasp position? [y/n]: " << std::endl;
    std::cout << std::setw(40) << std::setfill('=') << "\n";
    std::cout << std::setw(0);

    yarp::os::Bottle* replyPtr = userInputPort.read();
    while(!userConfirmsAction(replyPtr) )
    {
        std::cout << "Got a negative response. Waiting till you confirm action..." << std::endl;
        replyPtr->clear();
        replyPtr = userInputPort.read();
    }
}


void Experiment::setInitialWaypoints()
{
    // rHandPosStart = rightHandTask->getTaskFramePosition();
...
void Experiment::initializeOptimization()
{
    // setInitialWaypoints();
    /*
    *   Send opt params
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Experiment::userConfirmsAction(yarp::os::Bottle* reply)
{
    if(reply->size() == 1){
        std::string replyString = reply->get(0).asString();
        bool isYesReply = false;
        isYesReply = isYesReply || replyString == "y";
        isYesReply = isYesReply || replyString == "Y";
        isYesReply = isYesReply || replyString == "yes";
        isYesReply = isYesReply || replyString == "Yes";
        isYesReply = isYesReply || replyString == "YES";
...
void Experiment::doUpdate(double time, wocra::wOcraModel& state, void** args)
{
    ...
    if(movingToWeight)
    {
        ...
        if (!waitingForUser) {
            ...
            if (attainedGoal(state))
            {
                if (!messageSentOnce) {
                    ...
                    std::cout << std::setw(40) << std::setfill('=') << "\n";
                    std::cout << "Begin trials? [y/n]: " << std::endl;
                    std::cout << std::setw(40) << std::setfill('=') << "\n";
        ...
        {
            yarp::os::Bottle* replyPtr = userInputPort.read(false);
            if (replyPtr != NULL) {
                movingToWeight = !userConfirmsAction(replyPtr);
                if (!movingToWeight) {
                    messageSentOnce = false;
                    waitingForUser = false;
                }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if(robotIsStable)
                            {
                                ...
                                {
                                    std::cout << std::setw(40) << std::setfill('=') << "\n";
                                    std::cout << "Begin test number "<< testNumber + 1 <<" ? [y/n]: " << std::endl;
                                    std::cout << std::setw(40) << std::setfill('=') << "\n";
                                ...
                                if (replyPtr != NULL)
                                {
                                    if (userConfirmsAction(replyPtr))
                                    {
                                        dataSent_AwaitReply = false;
                                        waitForSolver = false;
                                        newOptVarsReceived = false;
...
void Experiment::initializeTrajectory(double time)
{
    resetTimeRight = time;
    initTrigger = false;
    costIterCounter = 0;
...
Eigen::VectorXd Experiment::mapVarianceToWeights(Eigen::VectorXd& variance)
{
...
double Experiment::postProcessInstantaneousCosts()
{
...
void Experiment::connectYarpPorts()
{
    ...
    std::string userInputPort_name = "/userInput:i";
    userInputPort.open(userInputPort_name.c_str());
    ...
    connectionElapsedTime = 0.0;
    while (!yarp.connect( "/user:o", userInputPort_name.c_str()) && connectionElapsedTime <= connectionTimeOut ){
        std::cout << "Waiting to connect to user input port. Please make sure you have opened a writer port named '/user:o'." << std::endl;
        yarp::os::Time::delay(waitInterval);
        connectionElapsedTime += waitInterval;
    }
...
void Experiment::sendFramePositionsToGazebo()
{
...
bool Experiment::sendTestDataToSolver()
{
...
bool Experiment::parseNewOptVarsBottle()
{
...
void Experiment::sendOptimizationParameters()
{
...
void Experiment::bottleEigenVector(yarp::os::Bottle& bottle, const Eigen::VectorXd& vecToBottle, const bool encapsulate)
{
    bottle.clear();
...
void Experiment::checkSolverStatus()
{
    setInitialWaypoints();
...
bool Experiment::openLogFiles(const std::string testLogFilePathPrefix)
{
    bool retVal = true;
    if (logTrajectoryData)
    {
