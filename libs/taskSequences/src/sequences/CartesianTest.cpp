...
// namespace sequence{
    void CartesianTest::doInit(ocra::Controller& ctrl, ocra::Model& model)
    {
        ...
        taskManagers["tmLeftHandCart"] = new ocra::SegCartesianTaskManager(ctrl, model, "leftHandCartesianTask", "l_hand", ocra::XYZ, Kp, Kd, wLeftHandTask, desiredPos);

    }

    void CartesianTest::doUpdate(double time, ocra::Model& state, void** args)
    {
        ocra::SegCartesianTaskManager*   tmp_tmLeftHandCart = dynamic_cast<ocra::SegCartesianTaskManager*>(taskManagers["tmLeftHandCart"]);
