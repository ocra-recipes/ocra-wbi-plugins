...
basicWholeBodyInterfaceModule::basicWholeBodyInterfaceModule()
{
    ctrlThread = 0;
    robotInterface = 0;
    period = 10;
...
bool basicWholeBodyInterfaceModule::configure(ResourceFinder &rf)
{
    ...
    ctrlThread = new basicWholeBodyInterfaceThread(moduleName, robotName, period, robotInterface, controller_options);
    if(!ctrlThread->start()){ fprintf(stderr, "Error while initializing locomotion control thread. Closing module.\n"); return false; }

    fprintf(stderr,"basicWholeBodyInterface thread started\n");

    return true;
}


bool basicWholeBodyInterfaceModule::interruptModule()
{
    if(ctrlThread)
        ctrlThread->suspend();
    return true;
...
bool basicWholeBodyInterfaceModule::close()
{
//stop threads
...
bool basicWholeBodyInterfaceModule::updateModule()
{
    if (ctrlThread==0)
    {
...
double basicWholeBodyInterfaceModule::getPeriod()
{
    return period;
}
