...
bool observerModule::configure(ResourceFinder &rf)
{
    ...
    {
        fprintf(stderr, "[ERR] observerModule: Impossible to open wholeBodyInterface: wbi_conf_file option missing");
    }
    ...
    {
        fprintf(stderr, "[ERR] observerModule: Impossible to load wbiId joint list with name %s\n", robotJointsListName.c_str());
    }
    ...
    ctrlThread = new observerThread(moduleName,
                                   robotName,
                                   period,
                                   robotInterface,
                                   controller_options,
    ...
    if(!ctrlThread->start()){ fprintf(stderr, "Error while initializing ISIRObservers thread. Closing module.\n"); return false; }

    fprintf(stderr,"ISIRObservers thread started\n");
